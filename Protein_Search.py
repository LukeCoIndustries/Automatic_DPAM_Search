...
your_data = "TuranovTabl1S1SortedbyScores.xlsx"

#What do you want you output file to be named (replace [insert here])
output_file = "top100.xlsx"
...
Z_score_column = 5 #change to your column
...
organize = "yes" #yes or now (please type in all lowercase)
...
your_data_df= your_data_df.sort_values(by=your_data_df.columns[4], ascending=False)
your_data_only_top_number_df = your_data_df.head(number_of_proteins + 1)
...
your_data_onlyUniProt_df = your_data_only_top_number_df.columns[Uniprot_column]
...
for value in your_data_onlyUniProt_df:
    if value not in Your_data_with_classifications_df:
